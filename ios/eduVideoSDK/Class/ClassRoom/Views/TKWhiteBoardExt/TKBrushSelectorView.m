...
- (void)showOnMiniWhiteBoardAboveView:(UIView *)view type:(TKBrushToolType)type
{
    ...
    [self mas_updateConstraints:^(MASConstraintMaker *make) {
        if (type == TKBrushToolTypeLine) {
            make.centerX.equalTo(view.mas_centerX).offset(95 * iphonePropor);
        } else if (type == TKBrushToolTypeText) {
            make.centerX.equalTo(view.mas_centerX).offset(35 * iphonePropor);
        } else {
